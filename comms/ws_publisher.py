...
def ws_pub(tracker_obj, (acc, vel, pwr, pos, force)):
    if isinstance(tracker_obj['lift_start'], type(dt.now())):
        tracker_obj['lift_start'] = dt.strftime(tracker_obj['lift_start'], '%Y-%m-%d %H:%M:%S')
